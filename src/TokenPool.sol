...
abstract contract TokenPool is IPoolV1, Ownable2StepMsgSender {
    ...
    function _consumeOutboundRateLimit(uint64 remoteChainSelector, uint256 amount) internal {
        ...
        emit OutboundRateLimitConsumed({
    ...
    function _onlyOnRamp(uint64 remoteChainSelector) internal view virtual {
    ...
    /// situation this function must be overridden and the ramp-check removed and replaced with a different
    /// access-control scheme.
    function _onlyOffRamp(uint64 remoteChainSelector) internal view virtual {
        if (!isSupportedChain(remoteChainSelector)) revert ChainNotAllowed(remoteChainSelector);
